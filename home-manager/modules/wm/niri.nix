...
spawn-at-startup "${config.xdg.configFile."river/autostart.sh".target}"
...
binds {
    ...
    Mod+F { spawn "${lib.getExe config.programs.firefox.package}"; }
    Mod+R { spawn "${lib.getExe pkgs.fuzzel}"; }
    Mod+E { spawn "${pkgs.foot}/bin/footclient -e \"${lib.getExe pkgs.yazi}\""; }
